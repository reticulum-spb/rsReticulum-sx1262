...
static const cyaml_schema_field_t config_fields[] = {
    ...
    CYAML_FIELD_MAPPING_PTR("rx_en", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                            plugin_config_t, rx_en, gpio_fields),
    CYAML_FIELD_MAPPING_PTR("tx_en", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                            plugin_config_t, tx_en, gpio_fields),
...
bool config_parse(const uint8_t *data, size_t len, plugin_config_t **out,
                  char *error, size_t error_size) {
    cyaml_err_t result;
    if (!out || !error || error_size == 0 || (!data && len)) return false;
    *out = NULL;
    result = cyaml_load_data(data ? data : (const uint8_t *)"{}", data ? len : 2,
                             &yaml_config, &config_schema, (cyaml_data_t **)out, NULL);
    if (result != CYAML_OK) {
    ...
    if (!*out || !(*out)->spi || (*out)->frequency < 150000000 || (*out)->frequency > 960000000 ||
        ...
        config_free(*out); *out = NULL; return false;
    }
    if ((*out)->preamble_symbols == 0) (*out)->preamble_symbols = 25;
    if ((*out)->sync_word == 0) (*out)->sync_word = UINT16_C(0x1424);
    if ((*out)->tcxo_voltage == 0.0) (*out)->tcxo_voltage = 1.8;
...
void config_free(plugin_config_t *config) {
    if (config) cyaml_free(&yaml_config, &config_schema, (cyaml_data_t *)config, 0);
}
